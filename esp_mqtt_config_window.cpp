...
void ESP_MQTT_Config_Window::JsonObjectToPlaintText(QJsonObject json_object, bool send_to_uart)
{
...
void ESP_MQTT_Config_Window::update_serial_data_to_plaint_text()
{
    ...
    {
        if(ui->data_from_comport_auto_scroll_checkBox->isChecked())
        {
        ...
        {
           long long current_position =  ui->data_from_com_port_plainTextEdit->verticalScrollBar()->value();
           ui->data_from_com_port_plainTextEdit->verticalScrollBar()->setValue(ui->data_from_com_port_plainTextEdit->verticalScrollBar()->maximum());
           ui->data_from_com_port_plainTextEdit->insertPlainText(QString(com_port_manager->readString()));
           ui->data_from_com_port_plainTextEdit->verticalScrollBar()->setValue(current_position);
        }
    }
}
...
void ESP_MQTT_Config_Window::on_broker_ip_pushButton_clicked()
{
    ...
    JsonObjectToPlaintText(temp_json_obj, true);
}
...
{
    QJsonObject temp_json_obj;
    (temp_json_obj)[BC_MQSC_COMMAND] = SETTING_CONNECT_PACKET;
...
{
    QJsonObject temp_json_obj;
    (temp_json_obj)[BC_MQSC_COMMAND] = SETTING_SUBSCRIBE_PACKET;

    QJsonArray subscribe_packet_list;
    for(uint8_t i = 0; i < NUM_OF_SUBSCRIBE_PACKET; i++)
    {
        QJsonObject subscribe_packet_obj;
        if(i == ui->subscribe_packet_index_lineEdit->text().toInt())
        {
            subscribe_packet_obj["topic_filter"] = ui->topic_filter_lineEdit->text();
        }

        subscribe_packet_list.insert(i, subscribe_packet_obj);
    }

    (temp_json_obj)["subscribe_packet_list"] = subscribe_packet_list;
...
void ESP_MQTT_Config_Window::on_max_qos_pushButton_clicked()
{
    ...
    for(uint8_t i = 0; i < NUM_OF_SUBSCRIBE_PACKET; i++)
    {
        ...
        if(i == ui->subscribe_packet_index_lineEdit->text().toInt())
        {
            subscribe_packet_obj["max_QoS"] = ui->max_qos_comboBox->currentText().toInt();
        }
...
void ESP_MQTT_Config_Window::on_broker_info_setting_to_text_clicked()
{
    ...
    JsonObjectToPlaintText(temp_json_obj, false);
}
...
void ESP_MQTT_Config_Window::on_start_up_general_setting_checkBox_stateChanged(int arg1)
{
    ...
    if(ui->general_setting_without_authentication_checkBox->isChecked())
    {
        ui->user_flag_checkBox->setChecked(false);
        ui->password_flag_checkBox->setChecked(false);
    }
    else
    {
    ...
    while(ex_str.indexOf(BC_MQSC_COMMAND) != -1)
    {
        ...
        if(str.indexOf(BC_MQSC_COMMAND) != -1)
        {
            ...
            if(!json_doc.isNull())
            {
                ...
                else if(command == QString(SETTING_CONNECT_PACKET))
                {
                ...
                else if(command == QString(SETTING_SUBSCRIBE_PACKET))
                {
                    json_obj[BC_MQSC_COMMAND].toString();
                    if(json_obj.contains("subscribe_packet_list"))
                    {
                        QJsonArray subscribe_packet_list = json_obj["subscribe_packet_list"].toArray();

                        for(uint8_t i = 0; i < NUM_OF_SUBSCRIBE_PACKET; i++)
                        {
                            QJsonObject subscribe_packet_json_obj = subscribe_packet_list[i].toObject();

                            if(i == ui->subscribe_packet_index_lineEdit->text().toInt())
                            {
                                if(subscribe_packet_json_obj.contains("max_QoS"))
                                {
                                    ui->max_qos_comboBox->setCurrentText(QString(QString::number(subscribe_packet_json_obj["max_QoS"].toInt())));
                                }

                                if(subscribe_packet_json_obj.contains("topic_filter"))
                                {
                                    ui->topic_filter_lineEdit->setText(subscribe_packet_json_obj["topic_filter"].toString());
                                }
                            }
                        }
                    }
                ...
                else if(command == QString(SETTING_PUBLISH_PACKET))
                {
                ...
                else if(command == QString(SETTING_ACTIVITY_CONFIG))
                {
                    ...
                    {
                        ui->trigger_pin_comboBox->setCurrentText(QString(QString::number(json_obj["_publish_trigger_pin"].toInt())));
                        qDebug() << QString(QString::number(json_obj["_publish_trigger_pin"].toInt()));
                    }
...
{
    QJsonObject temp_json_obj;
    (temp_json_obj)[BC_MQSC_COMMAND] = SETTING_PUBLISH_PACKET;
...
void ESP_MQTT_Config_Window::on_trigger_pin_pushButton_clicked()
{
    QJsonObject temp_json_obj;
    (temp_json_obj)[BC_MQSC_COMMAND] = SETTING_ACTIVITY_CONFIG;
    (temp_json_obj)["_publish_trigger_pin"] = ui->trigger_pin_comboBox->currentText().toInt();
...
void ESP_MQTT_Config_Window::on_publish_mode_comboBox_currentTextChanged(const QString &arg1)
{
    if(arg1 == QString("Continous"))
    {
        ...
        ui->continous_interval_pushButton->setEnabled(true);
        ui->trigger_type_comboBox->setEnabled(false);
        ui->trigger_type_pushButton->setEnabled(false);
        ui->trigger_pin_comboBox->setEnabled(false);
        ui->trigger_pin_pushButton->setEnabled(false);
    }
    else if(arg1 == QString("Trigger"))
    {
        ui->continous_interval_lineEdit->setEnabled(false);
        ui->continous_interval_pushButton->setEnabled(false);
        ui->trigger_type_comboBox->setEnabled(true);
        ui->trigger_type_pushButton->setEnabled(true);
        ui->trigger_pin_comboBox->setEnabled(true);
        ui->trigger_pin_pushButton->setEnabled(true);
    }
}
...
void ESP_MQTT_Config_Window::on_get_connect_properties_pushButton_clicked()
{
    QJsonObject temp_json_obj;
    (temp_json_obj)[BC_MQSC_COMMAND] = BC_MQSC_GET_PROPERTIES;
    (temp_json_obj)[BC_MQSC_PROPERTIES] = CONNECT_PACKET_PROPERTIES;
...
void ESP_MQTT_Config_Window::on_get_subscribe_properties_pushButton_clicked()
{
    QJsonObject temp_json_obj;
    (temp_json_obj)[BC_MQSC_COMMAND] = BC_MQSC_GET_PROPERTIES;
    (temp_json_obj)[BC_MQSC_PROPERTIES] = SUBSCRIBE_PACKET_PROPERTIES;
...
void ESP_MQTT_Config_Window::on_get_publish_properties_pushButton_clicked()
{
    QJsonObject temp_json_obj;
    (temp_json_obj)[BC_MQSC_COMMAND] = BC_MQSC_GET_PROPERTIES;
    (temp_json_obj)[BC_MQSC_PROPERTIES] = PUBLISH_PACKET_PROPERTIES;
...
void ESP_MQTT_Config_Window::on_configuration_data_clear_all_pushButton_clicked()
{
    ui->configuration_data_plainTextEdit->setPlainText("");
}

void ESP_MQTT_Config_Window::on_data_from_com_port_clear_all_pushButton_clicked()
{
    ui->data_from_com_port_plainTextEdit->setPlainText("");
}

void ESP_MQTT_Config_Window::on_general_setting_without_authentication_checkBox_stateChanged(int arg1)
{
    ui->user_flag_checkBox->setChecked(!bool(arg1));
    ui->password_flag_checkBox->setChecked(!bool(arg1));
}
